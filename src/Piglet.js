...
class Piglet {
  ...
  applyTypeRules() {
    if (this.measuredIn == "seconds") this.measuredIn = "s";
    if (this.measuredIn == "s") this.type = "time";
    if (this.measuredIn == "samples") this.type = "time";

    if (this.measuredIn == "dB") this.type = "gain";

    if (this.measuredIn == "Hz") this.type = "frequency";
    ...
    if (this.type == "midi") this.measuredIn = "semitones";
  }

  exposeDataToUnit() {
    if (this.exposeAsMono)
      this.unit[this.name] = this.signalChunk.channelData[0];
    else this.unit[this.name] = this.signalChunk.channelData;
  }
